...
function requestSwap(){
  ...
  if(!worker || !selectedDate){
    alert('이름과 날짜를 선택해 주세요.');
    return;
  }
  const msg = `안녕하세요 😊\n${selectedDate} ${worker} 근무 교체 가능하실까요?`;
  window.open('https://share.kakao.com/?text=' + encodeURIComponent(msg), '_blank');
}

function shareKakao(){
  window.open('https://share.kakao.com/?url=' + encodeURIComponent(location.href), '_blank');
}

function exportPDF(){
  html2pdf().from(document.querySelector('.calendar')).save();
}
